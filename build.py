...
def build_executable(current_platform):
    ...
    if current_platform == "windows":
        ...
        script_path = os.path.join("archive_downloader", "archive_downloader.py")
    ...
    PyInstaller.__main__.run(
        [
            ...
            "--onefile",
            "--exclude-module",
            "__init__",  # Exclude __init__.py
            script_path,
            "--distpath=dist/{}".format(
                current_platform
            ),  # Set output to a separate folder based on platform
        ]
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        if platform_arg in ["windows", "linux", "macos"]:
            ...
            print(
                f"Unsupported platform: {platform_arg}. Supported platforms: windows, linux, macos"
            )
    else:
